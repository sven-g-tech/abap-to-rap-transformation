...
*&---------------------------------------------------------------------*
CLASS z_lcl_ssvg_controller DEFINITION CREATE PRIVATE.
  PUBLIC SECTION.
    CLASS-METHODS get_instance
      RETURNING VALUE(r_instance) TYPE REF TO z_lcl_ssvg_controller.
  ...
  PRIVATE SECTION.
    CLASS-DATA lo_instance TYPE REF TO z_lcl_ssvg_controller.

    DATA lo_model TYPE REF TO z_lcl_ssvg_model.
    DATA lo_view  TYPE REF TO z_lcl_ssvg_view.
...
CLASS z_lcl_ssvg_controller IMPLEMENTATION.
  METHOD constructor.
    lo_model = NEW z_lcl_ssvg_model( ).
    lo_view = NEW z_lcl_ssvg_view( ).
  ENDMETHOD.

  METHOD get_instance.
    IF lo_instance IS NOT BOUND.
      lo_instance = NEW z_lcl_ssvg_controller( ).
    ENDIF.
